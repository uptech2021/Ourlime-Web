...
export default function VolunteerJobs({ job }: { job: Job[] }) {
	const volunteerJobs = job.filter((job) => job.type === 'Volunteer');

	return (
		<div>
			{volunteerJobs.length === 0 ? (
				<p className="text-center">No volunteer jobs found</p>
			) : (
