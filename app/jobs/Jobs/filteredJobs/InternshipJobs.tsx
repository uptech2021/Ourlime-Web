...
export default function InternshipJobs({ job }: { job: Job[] }) {
	const internshipJobs = job.filter((job) => job.type === 'Internship');

	return (
		<div>
			{internshipJobs.length === 0 ? (
				<p className="text-center">No internship jobs found</p>
			) : (
